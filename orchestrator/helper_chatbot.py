import paho.mqtt.client as mqtt
from typing import Any
import streamlit as st
import os
import sys
import json
import time
from tools import launch_agents_from_workflow

# Add the parent directory folder to find our modules
root_path = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
sys.path.append(root_path)

os.environ["OLLAMA_ACCELERATE"] = "gpu"


def assistant_response_callback(client: mqtt.Client, userdata: Any, msg: mqtt.MQTTMessage) -> None:
    """Callback function for when a message is received on the subscribed topic.

    Args:
        client (mqtt.Client): The MQTT client instance.
        userdata (Any): User-defined data of any type.
        msg (mqtt.MQTTMessage): The received MQTT message.
    """
    # Decode the message payload
    payload = msg.payload.decode('utf-8')
    data = json.loads(payload)
    # Store the assistant response in the session state
    st.session_state.chatbot_page_data["assistant_response"] = data["assistant_response"].replace(
        '\n', '\n\n')
    st.session_state.chatbot_page_data["new_chatbot_message"] = True


def chatbot_ui():
    """Streamlit UI for the chatbot agent."""
    # Initialize the control variables in the UI
    if not st.session_state.chatbot_page_data:
        st.session_state.chatbot_page_data = {}
        # Initialize the proper agents from the workflow
        workflow = st.session_state.workflows["chatbot"]
        # Call each agent docker to start
        launch_agents_from_workflow(
            workflow=workflow, agents=st.session_state.agents)
        # Subscribe to the chatbot output topic
        st.session_state.chatbot_page_data["input_topic"] = workflow["input_topics"][0]
        st.session_state.chatbot_page_data["output_topic"] = workflow["output_topics"][0]
        st.session_state.mqtt_client.subscribe(
            st.session_state.chatbot_page_data["output_topic"], qos=1)
        # Create the chatbot callback, controlling message flow with a flag
        st.session_state.mqtt_client.on_message = assistant_response_callback
        st.session_state.chatbot_page_data["new_chatbot_message"] = False
        # Initialize chat history
        st.session_state.chatbot_page_data["messages"] = []
        # Assistant response
        st.session_state.chatbot_page_data["assistant_response"] = ""

    # Display the app title and description
    st.title("Assistente de IA do GRIn")

    # Display chat messages from history on app rerun
    for message in st.session_state.chatbot_page_data["messages"]:
        with st.chat_message(message["role"]):
            st.markdown(message["content"])

    # React to user input
    if prompt := st.chat_input("Digite sua mensagem."):
        with st.chat_message("user"):
            st.markdown(prompt)
        # Publish the user input to the MQTT broker
        st.session_state.mqtt_client.publish(
            st.session_state.chatbot_page_data["input_topic"],
            payload=json.dumps({"user_input": prompt}),
            qos=1
        )
        # Run a spinner while waiting for the assistant response
        with st.spinner("Aguardando resposta do assistente..."):
            # Wait for the assistant response
            while not st.session_state.chatbot_page_data["new_chatbot_message"]:
                st.session_state.mqtt_client.loop(timeout=1.0)
            st.session_state.chatbot_page_data["new_chatbot_message"] = False
        # Get the assistant response from the MQTT broker
        response_placeholder = st.empty()
        typed_response = ""
        for char in st.session_state.chatbot_page_data["assistant_response"]:
            typed_response += char
            response_placeholder.markdown(typed_response)
            # Simulate typing effect
            time.sleep(0.01)
        # Store the assistant response in the session state
        st.session_state.chatbot_page_data["messages"].append(
            {"role": "user", "content": prompt})
        st.session_state.chatbot_page_data["messages"].append(
            {"role": "assistant", "content": st.session_state.chatbot_page_data["assistant_response"]})
