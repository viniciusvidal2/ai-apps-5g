...
def _edt_cv2_fallback(data: torch.Tensor) -> torch.Tensor:
    """Fallback EDT using OpenCV, moving data through CPU when Triton is unavailable."""
    import cv2

    assert data.dim() == 3
    device = data.device
    masks = data.detach().to("cpu").numpy().astype(np.uint8)
    output = np.empty(masks.shape, dtype=np.float32)
    for idx, mask in enumerate(masks):
        output[idx] = cv2.distanceTransform(mask, cv2.DIST_L2, 0)
